  ...
  switch (method) {
    case "POST":
      try {
        ...
        console.log("this is meetingRoom ", meetingRoomExists);
        if (meetingRoomExists && meetingRoomExists?.MeetingId) {
          await chime
            ...
            .then(async (response) => {
              ...
              if (
                (attendees.length == 1 && attendees[0].ExternalUserId?.includes("MediaPipeline")) ||
                attendees.length == 0
              ) {
                response = await chime
                  .deleteMeeting({ MeetingId: meetingRoomExists?.MeetingId?.toString() || "" })
                  .promise()
                  .then((r) => r)
                  .catch((e) => e);
              }
